...
- Public `juanpager/renderer`, `juanpager/state`, `juanpager/interaction`, and `juanpager/session` package entrypoints.
...
- Executable tests for projection rendering, viewports, hierarchy expansion, clocks, transaction conflicts, M1 state replay, durable sessions, MCP completion, undo, and redo.
...
- Universal state values and durable sessions remain bounded data and cannot carry markup, code, callbacks, components, or renderer instructions.
...
- Durable sessions wrap the same JuanPage and optional M1 session; they are delivery and lifecycle storage, not another presentation format.
