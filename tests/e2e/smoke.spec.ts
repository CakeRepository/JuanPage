...
test("one semantic schema opens in one adaptive renderer", async ({ page, context }) => {
  ...
  await expect(viewer.getByRole("main")).toContainText("Meaning is the Interface");
  await expect(viewer.getByRole("heading", { name: "Meaning is the Interface" })).toHaveCount(1);
