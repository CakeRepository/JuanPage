...
async function main(): Promise<void> {
  ...
  if (!input) {
    console.error('Usage: npm run decode -- "https://example/#v=5&enc=gz&data=..."');
    process.exit(1);
  }
  let hash = input;
  try {
    hash = new URL(input).hash || input;
  } catch {
    // Accept a raw URL fragment.
  }
  const { data, version, encoding } = parseFragment(hash);
  if (!data) throw new Error("No data payload found");
  if (version && version !== "5") throw new Error(`Unsupported fragment version v=${version}; expected v=5.`);
  ...
  console.error(decoded.kind === "m1" ? "Decoded untrusted M1 packet." : "Decoded JuanPage 2.0.");
