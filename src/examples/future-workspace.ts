...
export const futureWorkspace: JuanPageDocument = {
  ...
  objects: [
    ...
    { id: "schema", type: "capability", group: "System", name: "JuanPage object graph", status: "Ready", tone: "info", interaction: "inspect", summary: "Arbitrary objects, ordered fields, relationships, actions, metrics, and view preferences in one strict document.", fields: [{ key: "version", value: "1.0" }, { key: "objectsSupported", value: "Any typed object" }, { key: "complete", value: true }] },
    { id: "runtime", type: "capability", group: "System", name: "Adaptive runtime", status: "Ready", tone: "info", interaction: "inspect", summary: "The same objects become a canvas, a dense data surface, or a relationship flow without regenerating the source.", fields: [{ key: "lenses", value: ["Canvas", "Data", "Flow"] }, { key: "trustedDOM", value: true }, { key: "complete", value: true }] },
    ...
    { id: "credits", type: "resource", group: "Resources", name: "Runtime compute credits", status: "Available", tone: "neutral", interaction: "display", summary: "Resources are just objects too. The renderer does not need a new component type.", fields: [{ key: "credits", value: 4200, format: "number", display: "prominent" }, { key: "unit", value: "agent-seconds" }, { key: "complete", value: false }] },
    { id: "risk", type: "risk", group: "Signals", name: "Interface fragmentation", status: "Eliminated", tone: "success", interaction: "display", summary: "The system no longer accumulates one schema and one renderer for every use case.", fields: [{ key: "severity", value: "Previously high" }, { key: "mitigation", value: "Universal graph + adaptive lenses" }, { key: "complete", value: true }] }
  ],
  relations: [
  ...
  actions: [
    ...
    { id: "renderer-effort", kind: "number", label: "Effort", target: "task-renderer", field: "effort", initial: 5, min: 0, max: 20, step: 1, control: "range" }
  ]
};
