...
export type PageActionInvocation = Readonly<{
  actionId: string;
...
export type UniversalRenderOptions = {
  ...
  onAction?: (invocation: PageActionInvocation) => void | Promise<void>;
};
...
function metricValue(metric: PageMetric, page: JuanPageDocument, state: PageState): string {
  ...
  if (metric.operation === "sum-product") value = objects.reduce((total, object) => {
...
export function renderPage(page: JuanPageDocument, mount: HTMLElement, options: UniversalRenderOptions = {}): RenderHandle {
  ...
  const state = loadPageState(key);
  const actionMap = new Map((page.actions ?? []).map((action) => [action.id, action]));
  let query = "";
  const persist = (): void => savePageState(key, state);
  const setLens = (lens: PageLens): void => { state.lens = lens; persist(); draw(); };

  const notifyAction = async (action: PageAction, object?: PageObject): Promise<void> => {
    if (!options.onAction) return;
    const target = "target" in action ? action.target : object?.id;
  ...
  const runCopy = async (action: Extract<PageAction, { kind: "copy" }>, object?: PageObject): Promise<void> => {
    ...
    if (action.source === "field" && action.target && action.field) {
      const target = pageObject(page, action.target);
      if (target) text = scalarText(objectValue(target, action.field, state));
    }
  ...
  const actionControl = (action: PageAction, object?: PageObject): HTMLElement => {
    ...
    const host = el("div", { className: "jp-u-action", attrs: { "data-action-id": action.id } });

    if (action.kind === "open") {
      const link = externalLink(action.url, action.label, `jp-u-button jp-u-${action.tone ?? "neutral"}`);
      link.setAttribute("data-action-id", action.id);
      link.addEventListener("click", () => { void notifyAction(action, object); });
      return link;
    }

    if (action.kind === "copy" || action.kind === "emit") {
      const button = el("button", {
        className: `jp-u-button jp-u-${action.tone ?? "neutral"}`,
        text: action.label,
        attrs: { type: "button", "data-action-id": action.id },
      });
      button.addEventListener("click", () => {
        void (async () => {
          if (action.kind === "copy") {
            await runCopy(action, object);
            announce(host, "Copied");
          } else {
            await notifyAction(action, object);
            announce(host, "Request sent");
          }
        })();
      });
      return button;
    ...
    const label = el("label", { className: "jp-u-action-label" });
    append(label, el("span", { text: action.label }));
    const current = effectiveFieldValue(targetObject, action.field, state, "initial" in action ? action.initial : undefined);

    if (action.kind === "toggle") {
      const input = el("input", { attrs: { type: "checkbox", "data-action-id": action.id } }) as HTMLInputElement;
      ...
      input.addEventListener("change", () => {
        ...
        setPageValue(state, targetObject.id, action.field, next);
        persist();
        draw();
      });
      ...
      select.value = String(current ?? action.initial ?? action.options[0].value);
      select.addEventListener("change", () => { setPageValue(state, targetObject.id, action.field, select.value); persist(); draw(); });
      append(label, select);
    } else if (action.kind === "text") {
      const input = action.multiline
        ? el("textarea", { attrs: { placeholder: action.placeholder ?? "", "data-action-id": action.id } }) as HTMLTextAreaElement
        : el("input", { attrs: { type: "text", placeholder: action.placeholder ?? "", "data-action-id": action.id } }) as HTMLInputElement;
      input.value = String(current ?? action.initial ?? "");
      input.addEventListener("change", () => { setPageValue(state, targetObject.id, action.field, input.value); persist(); draw(); });
      append(label, input);
    }
    append(host, label);
    return host;
  ...
  const cardsLens = (objects: PageObject[]): HTMLElement => {
    const body = el("div", { className: "jp-u-groups" });
    const groupBy = page.view?.groupBy ?? "group";
    if (groupBy === "none") {
      const grid = el("div", { className: "jp-u-grid" });
      for (const object of objects) append(grid, card(object));
      append(body, grid);
      return body;
    }
    const buckets = new Map<string, PageObject[]>();
    for (const object of objects) {
      const name = groupBy === "type" ? object.type : groupBy === "status" ? object.status ?? "No status" : object.group ?? "Other";
      buckets.set(name, [...(buckets.get(name) ?? []), object]);
    }
    for (const [name, items] of buckets) {
      const section = el("section", { className: "jp-u-group" });
      append(section, el("h2", { text: name }), el("span", { className: "jp-u-count", text: String(items.length) }));
      const grid = el("div", { className: "jp-u-grid" });
  ...
  const tableLens = (objects: PageObject[]): HTMLElement => {
    const wrap = el("div", { className: "jp-u-table-wrap" });
    const table = el("table", { className: "jp-u-table" });
    const keys = [...new Set(objects.flatMap((object) => (object.fields ?? []).filter((field) => field.display !== "hidden").map((field) => field.key)))].slice(0, 8);
    const head = el("thead");
    const headRow = el("tr");
    for (const label of ["Name", "Type", "Status", ...keys.map(humanizeKey)]) append(headRow, el("th", { text: label }));
    append(head, headRow);
    table.append(head);
    const body = el("tbody");
    for (const object of objects) {
  ...
  const flowLens = (objects: PageObject[]): HTMLElement => {
    ...
    const root = el("div", { className: "jp-u-flow" });
    const lanes = new Map<string, PageObject[]>();
    ...
    for (const [name, items] of lanes) {
      const lane = el("section", { className: "jp-u-lane" });
      append(lane, el("h2", { text: name }));
      for (const object of items) append(lane, card(object));
      append(root, lane);
    }
    const relations = (page.relations ?? []).filter((relation) => ids.has(relation.from) && ids.has(relation.to));
    if (relations.length) {
      const list = el("section", { className: "jp-u-relations" });
      append(list, el("h2", { text: "Relationships" }));
      for (const relation of relations) append(list, el("p", { text: `${pageObject(page, relation.from)?.name ?? relation.from} → ${relation.label ?? humanizeKey(relation.kind)} → ${pageObject(page, relation.to)?.name ?? relation.to}` }));
      append(root, list);
    }
    return root;
  };
  ...
  const inspector = (object: PageObject): HTMLElement => {
    const panel = el("aside", { className: "jp-u-inspector", attrs: { "aria-label": `${object.name} details` } });
    const close = el("button", { className: "jp-u-close", text: "Close", attrs: { type: "button" } });
    close.addEventListener("click", () => { state.selectedId = undefined; persist(); draw(); });
    append(panel, close, el("p", { className: "jp-u-type", text: humanizeKey(object.type) }), el("h2", { text: object.name }));
    if (object.summary) append(panel, el("p", { className: "jp-u-summary", text: object.summary }));
    const details = el("dl", { className: "jp-u-details" });
    for (const field of object.fields ?? []) {
      if (field.display !== "hidden") append(details, el("dt", { text: field.label ?? humanizeKey(field.key) }), el("dd", { text: formatValue(objectValue(object, field.key, state), field) }));
    }
    append(panel, details);
    if (object.url) append(panel, externalLink(object.url, "Open source", "jp-u-button jp-u-primary"));
    const actions = (object.actionIds ?? []).map((id) => actionMap.get(id)).filter((action): action is PageAction => Boolean(action));
    if (actions.length) {
      const controls = el("div", { className: "jp-u-actions" });
      for (const action of actions) append(controls, actionControl(action, object));
      append(panel, controls);
    }
    return panel;
  };

  function draw(): void {
    const root = el("div", { className: `jp-u-page jp-u-density-${page.view?.density ?? "comfortable"}` });
    const header = el("header", { className: "jp-u-header" });
    const brand = el("div", { className: "jp-u-brandline" });
    append(brand, el("span", { className: "jp-u-brand", text: "JUAN" }), el("span", { text: "Universal human surface" }));
    const utility = el("div", { className: "jp-u-utility" });
    if (options.builderHref) append(utility, el("a", { className: "jp-u-button", text: "Build", attrs: { href: options.builderHref } }));
    ...
    const reset = el("button", { className: "jp-u-button", text: "Reset", attrs: { type: "button" } });
    reset.addEventListener("click", () => { resetPageState(key); state.values = {}; state.lens = undefined; state.selectedId = undefined; state.activeGroup = undefined; draw(); });
    append(utility, share, reset);
    append(header, brand, utility, el("h1", { text: page.title }));
    ...
    if (page.metrics?.length) {
      ...
      for (const metric of page.metrics) {
        const item = el("div", { className: "jp-u-metric" });
        append(item, el("strong", { text: metricValue(metric, page, state) }), el("span", { text: metric.label }));
        append(metrics, item);
      }
    ...
    const search = el("input", { attrs: { type: "search", placeholder: "Search the world", value: query, "aria-label": "Search objects" } }) as HTMLInputElement;
    ...
    if (groupBy !== "none") {
      ...
      const groups = [...new Set(page.objects.map((object) => groupBy === "type" ? object.type : groupBy === "status" ? object.status ?? "No status" : object.group ?? "Other"))];
    ...
    const lenses = el("div", { className: "jp-u-lenses", attrs: { role: "toolbar", "aria-label": "View" } });
    const current = state.lens ?? page.view?.defaultLens ?? "cards";
    for (const [lens, label] of [["cards", "Canvas"], ["table", "Data"], ["flow", "Flow"]] as const) {
      const button = el("button", { className: `jp-u-lens${current === lens ? " is-active" : ""}`, text: label, attrs: { type: "button", "aria-pressed": current === lens } });
      button.addEventListener("click", () => setLens(lens));
      append(lenses, button);
    }
    append(controls, lenses);

    const objects = visible();
    const workspace = el("main", { className: "jp-u-workspace" });
    if (!objects.length) append(workspace, el("p", { className: "jp-u-empty", text: "No objects match this view." }));
    else if (current === "table") append(workspace, tableLens(objects));
    else if (current === "flow") append(workspace, flowLens(objects));
